...
- Psychological test results are now persisted in `PsychologicalTestResult` through `/api/v1/psychological-tests/results`. Candidates can read their own results through `/api/v1/psychological-tests/results/me`; admin-session users can read all results through `/api/v1/admin/psychological-tests/results`. The frontend saves results when the full-screen runner finishes, shows them in the candidate account dashboard, and lists all results in the admin panel testing tab.
