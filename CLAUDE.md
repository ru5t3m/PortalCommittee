...
Before making conclusions or proposing changes, inspect the current code. This file captures durable project direction, but implementation may have advanced since it was last edited.

Use conservative engineering judgment:

- Prefer existing project patterns over new abstractions.
- Keep changes scoped to the requested feature or fix.
- Do not reintroduce demo behavior, mock credentials, or hardcoded secrets.
- When a change touches authentication, authorization, session handling, public forms, admin workflows, or storage, consider security and auditability explicitly.
- Keep Russian and Kazakh behavior consistent.
- Keep public-facing copy suitable for an official government portal.
...
- `/[locale]/psychological-testing` is the public intro page for the imported `primary-selection` test from `Первичный психологический отбор.docx`. It explains the sequential sections and links to `/[locale]/psychological-testing/primary-selection`.
- The `primary-selection` route is a full-screen test mode without the normal portal header/footer and requires an authenticated portal session via `/auth/me` before showing questions.
- The psychological test runner shows instructions before each section, pauses the one-hour timer while instructions are visible, and presents questions in pages of 10.
- The runner currently has 50 numeric questions and 50 visual/figure-selection questions.
- Related images live in `apps/web/public/psychological-tests/primary-selection`.
- Memory and interpretation sections are planned placeholders.
- Automatic scoring is intentionally disabled until answer keys are provided.
- Psychological test results are persisted in `PsychologicalTestResult` through `/api/v1/psychological-tests/results`.
- Candidates can read their own results through `/api/v1/psychological-tests/results/me`.
- Admin-session users can read all results through `/api/v1/admin/psychological-tests/results`.
- The frontend saves results when the full-screen runner finishes, shows them in the candidate account dashboard, and lists all results in the admin panel testing tab.
...
- Use structured APIs and typed schemas instead of ad hoc string manipulation where practical.
- Avoid unrelated refactors and formatting churn.
- If tests already exist for the touched area, update or extend them.

## Claude Code Workflow

When working in this repository:

1. Read the relevant files before deciding that a feature is missing.
2. Check current git status before broad edits.
3. Preserve user changes and unrelated work.
4. Use migrations for database schema changes.
5. Keep backend API, frontend types, and UI states aligned.
6. Run the narrowest useful verification for the changed area.
...
- Bypass the admin-session model.
- Trust ordinary user role alone for admin APIs.
- Add `/en`, `/mobile`, search, uploads, eGov, SMS, digital signature, or external identity-provider features unless explicitly requested.
- Use `Base.metadata.create_all()` in the FastAPI startup path.
- Use the legacy SQLite database for active development.
